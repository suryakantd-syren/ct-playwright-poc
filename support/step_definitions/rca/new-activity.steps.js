...
Given('User fill the definition details for new RCAModel', async function ({ context }) {
  let newPage = await dataUtils.getPage(context, 'CC');
  const newPageConstants = new PageConstants(newPage);
  await newPageConstants.activityDefinitionPage.fillRCAModelDefination();
});
...
Given('User save the new RCAModel', async function ({ context }) {
  let newPage = await dataUtils.getPage(context, 'CC');
  const newPageConstants = new PageConstants(newPage);
  //await this.page.waitForTimeout(30);
  await newPageConstants.activityDefinitionPage.saveRCAModel();
});

Then('User verifies RCAModel list page after save completion', async function ({ context }) {
  let newPage = await dataUtils.getPage(context, 'CC');
  const newPageConstants = new PageConstants(newPage);
  //await this.page.waitForTimeout(30);
